        ...
        */
            ...
            UUPSUpgradeable
        {
            ...
            IERC20Upgradeable public stakingToken; // The ERC20 token used for staking
            ...
            uint256 public BASE_SCORE_VALUE; // Base score value used as multiplier in score calculations
            ...
            function initialize(address _stakingToken, uint256 initialBalance) public initializer {
                __Ownable_init();
                ...
                stakingPeriods     = [0 days, 10 minutes, 3 * 30 days, 6 * 30 days, 12 * 30 days];
                stakingAPRs        = [30, 42, 60, 90, 120];
                stakingMultipliers = [0, 1000, 1500, 2200, 4000];
                tierScore[0] = 0;
                tierScore[1] = 100_000;
                tierScore[2] = 500_000;
                tierScore[3] = 2_000_000;
                tierScore[4] = 5_000_000;
                BASE_SCORE_VALUE = 1;

                // fund the contract and open it
                stakingToken.safeTransferFrom(msg.sender, address(this), initialBalance);
                isOpen = true;
            }
            ...
            */
            function initiateUnlock(uint256 _id) external view {
            ...
            */
            ...
            /// @notice Claim the accrued reward for a specific stake and restake it under a new lock
            /// @param _id           The ID of the original stake
            /// @param _stakingType  The lock type (0=flexible, 1..n=locked) for the reward
            function restakeRewardFromPosition(uint256 _id, uint256 _stakingType) external nonReentrant {
                require(_stakingType < stakingPeriods.length, "Invalid staking type");

                Stake storage s = stakes[_id];
                require(!s.finished, "Stake already withdrawn");
                require(s.user == msg.sender, "Not the stake owner");

                // 1) calculate reward so far (ignores lock check)
                uint256 rewardAmount = calculateReward(_id);
                require(rewardAmount > 0, "No rewards to restake");

                // 2) zero out the reward portion on the original position
                //    by resetting its startTime so future rewards start fresh
                s.startTime = block.timestamp;

                // 3) record the new reward‐stake
                stakes.push(
                    Stake({
            ...
            /// @notice Claim the accrued reward for a single stake, regardless of lock status
            function claimReward(uint256 _id) external nonReentrant {
                Stake storage s = stakes[_id];
                require(!s.finished, "Already withdrawn");
                require(s.user == msg.sender, "Not stake owner");

                uint256 rewardAmount = calculateReward(_id);
                require(rewardAmount > 0, "No rewards available");

                // zero out startTime so future calculateReward is zero until a restake/withdraw
                s.startTime = block.timestamp;
                ...
                require(
                  stakingToken.balanceOf(address(this)) >= rewardAmount,
                  "Insufficient contract balance"
                );
                stakingToken.safeTransfer(msg.sender, rewardAmount);
                // no change to s.finished or tokenAmount — principal remains staked
                emit Withdraw(_id, rewardAmount);
            }

            /// @notice Claim all available rewards across your stakes in one call
            function claimAllRewards() external {
                uint256[] memory ids = getStakeIdsByOwner(msg.sender);
                for (uint i = 0; i < ids.length; i++) {
                    // swallow errors so a zero-reward or out-of-balance on one doesn't revert the whole batch
                    try this.claimReward(ids[i]) {} catch {}
                }
            }
