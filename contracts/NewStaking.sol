        ...
        */
        contract Staking is Ownable, ReentrancyGuard {
            ...
            uint256 public constant MIN_RESTAKE_DURATION = 60; // 60 seconds = 1 minute
            ...
            mapping(address => uint256[]) public stakeIdsByUser; // Mapping of user address to their stake IDs
            ...
            function stakeTokens(
                ...
                stakes.push(
                    Stake({
                        ...
                        user: _user,
                        id: totalNumberOfStakes, // ID for the new stake
                        finished: false
                    })
                );
                stakeIdsByUser[_user].push(totalNumberOfStakes); // Track stake ID for user
            ...
            */
            function withdrawStake(uint256 _id) external nonReentrant {
                ...
                require(
                    contractBalance >= stake.tokenAmount + rewardAmount,
            ...
            */
            function restakeRewards(uint256 _id, uint256 _stakingType) external {
                ...
                uint256 stakingDuration = block.timestamp - stake.startTime;
                require(stakingDuration >= MIN_RESTAKE_DURATION, "Stake too new to restake");
                ...
                stakeIdsByUser[msg.sender].push(totalNumberOfStakes); // Track new stake ID for user

                totalNumberOfStakes++;
                totalAmountOfStakes = totalAmountOfStakes - stake.tokenAmount + newAmount;

                emit Restake(_id, _stakingType);
                emit Deposit(msg.sender, newAmount, _stakingType); // Emit for visibility
            }
            ...
            */
            function emergencyWithdrawUser(address user) external onlyOwner {
                ...
                uint256 contractBalance = stakingToken.balanceOf(address(this));
                require(
                    contractBalance >= totalRequiredPayout,
